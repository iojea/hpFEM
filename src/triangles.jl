...
"""
@inline edges(t::TriangleHP) = [EdgeHP(_eval(t, i), _eval(t, i + 1)) for i = 1:3]
...
function triangle(t::T, p::AbstractMatrix) where {T<:Union{Tuple,AbstractArray}}
    maxi = argmax(norm(p[:, t[mod1(i + 1, 3)]] - p[:, t[i]]) for i = 1:3)
...
TriangleProperties{P,F}(val, η, ηₚ) where {P,F} =
    TriangleProperties(Ref(P(val)), Ref(F(η)), Ref(F(ηₚ)))
TriangleProperties{P}(val, η, ηₚ) where {P} =
    TriangleProperties(Ref(P(val)), Ref(η), Ref(ηₚ))
TriangleProperties(val::P, η::F, ηₚ::F) where {P,F} =
    TriangleProperties(Ref(val), Ref(η), Ref(ηₚ))
TriangleProperties{P}() where {P} = TriangleProperties(P(0), 0.0, 0.0)
TriangleProperties() = TriangleProperties(Int8(0), 0.0, 0.0)
TriangleProperties(tp::TriangleProperties) = TriangleProperties(tp.refine, tp.η, tp.ηₚ)
TriangleProperties{P}(tp::TriangleProperties) where {P} =
    TriangleProperties(P(tp.refine[]), tp.η, tp.ηₚ)
TriangleProperties{P,F}(tp::TriangleProperties) where {P,F} =
    TriangleProperties(P(tp.refine[]), F(tp.η), F(tp.ηₚ))
