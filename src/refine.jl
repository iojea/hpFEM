...
RefAux{I,P}() where {I<:Integer,P<:Integer} =
    RefAux(MVector{6,P}(zeros(6)), MVector{6,I}(zeros(6)), Dictionary{EdgeHP{I},I}())
...
intriangle(p::T, vert::M) where {T<:AbstractArray,M<:AbstractArray} =
    intriangle(p, eachcol(vert)...)
...
function refine_red!(
    ...
    for j in eachindex(t_edges)
        ...
        if k > 0
            ...
            points[:, i[]] = sum(points[:, edge], dims = 2) / 2.0
            dots[j+3] = i[]
            set!(seen, edge, i[])
            ...
            set!(
                edgelist,
                EdgeHP{I}(edge[1], i[]),
                EdgeProperties{P,Bool}(degs[j], m, false),
            )
            set!(
                edgelist,
                EdgeHP{I}(i[], edge[2]),
...
function refine_blue!(
    ...
    if ismarked(edgelist[t_edges[2]])
        ...
        set!(
            edgelist,
            EdgeHP{I}(dots[3], dots[4]),
            EdgeProperties{P,Bool}(degs[4], 0, false),
        )
        set!(
            edgelist,
            EdgeHP{I}(dots[5], dots[4]),
            EdgeProperties{P,Bool}(degs[5], 0, false),
        )
        ...
        for j = 0:1
            ...
            if k > 0
                ...
                set!(
                    edgelist,
                    EdgeHP{I}(edge[1], i[]),
                    EdgeProperties{P,Bool}(degs[1+2j], m, false),
                )
                set!(
                    edgelist,
                    EdgeHP{I}(i[], edge[2]),
...
function refine_green!(
    ...
    if k > 0
        ...
        set!(
            edgelist,
            EdgeHP{I}(dots[1], dots[4]),
            EdgeProperties{P,Bool}(degs[1], 0, false),
        )
        set!(
            edgelist,
            EdgeHP{I}(dots[4], dots[2]),
            EdgeProperties{P,Bool}(degs[1], 0, false),
        )
        i[] += 1
    end
